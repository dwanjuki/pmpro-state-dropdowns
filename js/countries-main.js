jQuery(document).ready(function ($) {
	...
	if (jQuery('#bcountry').length) {
		...
		if (typeof states_for_countries !== 'undefined' && jQuery(states_for_countries).length > 0) {
			if (jQuery('#bstate').hasClass('pmpro_form_input-error')) {
				jQuery("#bstate").replaceWith('<select id="bstate" name="bstate" class="pmpro_form_input pmpro_form_input-select pmpro_form_input-error pmpro_form_input-required"></select>');  //convert to dropdown so states can auto-populate
			} else {
				jQuery("#bstate").replaceWith('<select id="bstate" name="bstate" class="pmpro_form_input pmpro_form_input-select pmpro_form_input-required"></select>');  //convert to dropdown so states can auto-populate
			}
			...
			if (jQuery('#bstate').hasClass('pmpro_form_input-error')) {
				jQuery("#bstate").replaceWith('<input type="text" id="bstate" name="bstate" class="pmpro_form_input pmpro_form_input-text pmpro_form_input-error" value="' + selected_state + '"/>');
			} else {
	...
	jQuery('body').on('change', "#bcountry", function () {
		...
		if (typeof selected_states !== 'undefined' && jQuery(selected_states).length > 0) {
			if (jQuery("#bstate").hasClass('pmpro_form_input-error')) {
				jQuery("#bstate").replaceWith('<select id="bstate" name="bstate" class="pmpro_form_input pmpro_form_input-select pmpro_form_input-required pmpro_form_input-error"></select>');  //convert to dropdown so states can auto-populate
			} else {
				jQuery("#bstate").replaceWith('<select id="bstate" name="bstate" class="pmpro_form_input pmpro_form_input-select pmpro_form_input-required"></select>');  //convert to dropdown so states can auto-populate
			}
			pmprosd_populate_dropdown("#bstate", selected_states, pmpro_state_labels.region, '');
	...
	if (jQuery('#pmpro_scountry').length) {
		...
		if (typeof selected_states !== 'undefined' && jQuery(selected_states).length > 0) {
			if (jQuery("#pmpro_sstate").hasClass('pmpro_form_input-error')) {
				jQuery('#pmpro_sstate').replaceWith('<select id="pmpro_sstate" name="pmpro_sstate" class="pmpro_form_input pmpro_form_input-select pmpro_form_input-required pmpro_form_input-error"></select>');  //convert to dropdown so states can auto-populate   
			} else {
				jQuery('#pmpro_sstate').replaceWith('<select id="pmpro_sstate" name="pmpro_sstate" class="pmpro_form_input pmpro_form_input-select pmpro_form_input-required"></select>');  //convert to dropdown so states can auto-populate   
			}
			...
			if (jQuery("#pmpro_sstate").hasClass('pmpro_form_input-error')) {
				jQuery("#pmpro_sstate").replaceWith('<input type="text" id="pmpro_sstate" name="pmpro_sstate" class="pmpro_form_input pmpro_form_input-text pmpro_form_input-error" value="' + selected_state + '" />');
			} else {
